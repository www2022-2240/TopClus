...
class TopClusModel(BertPreTrainedModel):
    ...
    def __init__(self, config, input_dim, hidden_dims, n_clusters, temp):
    ...
    def forward(self, input_ids, attention_mask, valid_pos):
        ...
        rec_emb, z = self.ae(doc_emb)
        p = self.cluster_assign(z)
        p_word = self.cluster_assign(z_word)
        dec_topic = self.ae.decode(self.topic_emb)
        rec_doc_emb = torch.matmul(p, dec_topic)
        return avg_doc_emb, rec_emb, z, p, input_embs, output_embs, rec_doc_emb, p_word
    ...
    def inference(self, input_ids, attention_mask):
        ...
        _, z = self.ae(doc_emb)
        p = self.cluster_assign(z)
        return doc_emb, z, p, valid_word_ids, sim

    def init_doc_emb(self, input_ids, attention_mask):
        self.bert.eval()
        bert_outputs = self.bert(input_ids,
                                 attention_mask=attention_mask)
        last_hidden_states = bert_outputs[0]
    ...
    def pretrain_forward(self, input_ids, attention_mask):
        self.bert.eval()
        bert_outputs = self.bert(input_ids,
                                 attention_mask=attention_mask)
        last_hidden_states = bert_outputs[0]
